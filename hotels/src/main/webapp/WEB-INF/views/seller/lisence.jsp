...
$(function() {
	...
	$(function(){
		$("form").submit(function(e){
			e.preventDefault();
// 			체크박스 상태값 가져와서 treu false 인지 체크해서
// 			체크되있으면 넘기고 아니면 
// 			alert 창 
// 			console.log($("input:checkbox[name=agree]").val())
			if($("input:checkbox[name=agree]").is(":checked")==true){
				this.submit();
			}
			else{
				window.alert("체크박스 동의에 체크하여 주세요")
			}
		});
	});
</script>
...
제1장 통칙
제1조 (목적)
본 약관은 주식회사 이베이코리아(이하 '회사'라 합니다)가 제공하는 전자금융거래서비스를 다음 각 호의 인터넷사이트 등(이하 ‘인터넷사이트’라 합니다)을 통하여 가입한 이용자가 온라인 또는 오프라인으로 이용하는 경우, 회사와 이용자 간 전자금융거래의 법률관계를 정함에 그 목적이 있습니다.
...
1.옥션 사이트의 경우
주소: 경기도 부천시 원미구 상동 461 투나빌딩 6층
이메일 주소:mediation@corp.auction.co.kr
전화번호: 1588-0184
2. G마켓 사이트의 경우
주소: 경기도 부천시 원미구 상동 461 투나빌딩 6층
이메일 주소: gmk_cs@corp.gmarket.co.kr
전화번호: 1566-5701
3. G9 사이트의 경우
주소: 경기도 부천시 원미구 상동 461 투나빌딩 6층
이메일 주소: g9@g9.co.kr
...
⑤회사는 제4항에도 불구하고 천재지변, 회사의 귀책사유가 없는 정전, 화재, 통신장애 기타의 불가항력적 사유로 처리불가능하거나 지연된 경우로서 이용자에게 처리불가능 또는 지연사유를 통지한 경우(금융기관 또는 결제수단 발행업체나 통신판매업자가 통지한 경우를 포함합니다) 또는 회사가 고의, 과실 없음을 입증한 경우에는 이용자에 대하여 손해배상책임을 지지 않습니다.
제11조 (분쟁처리 및 분쟁조정)
...
<br> <input type="checkbox" value="agree" name="agree" required="required"> 동의합니다
</div>
	<div class="lisence_box container">
        <form action="lisence" method="post">
            ...
            <input type="submit" class="btn btn-danger btn-block lisencebtn" name="lisencebtn" value="확인">
        </form>
	</div>
